...
class _ConversationDetailView(View):  # form을 이용할때
    def get(self, *args, **kwargs):
        pk = kwargs.get("pk")
        conversation = models.Conversation.objects.get_or_none(pk=pk)
        if not conversation:
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            u_message = form.cleaned_data.get("message")
        models.Message.objects.create(
            message=u_message, user=self.request.user, conversation=conversation
        )
        return redirect(reverse("conversations:detail", kwargs={"pk": pk}))
